...
def plot_lc_agg(scores, metric_name:str, prfx:str, tr_set:str='te', **plot_args):
    """ 
    Plots all LC data points from all data split runs, icnluding the median
    score per subset size.
    Args:
        scores : aggregated dataframe from all scores of multiple runs.
        prfx : prfx identifier of the scores columns

    Returns:
        ax :
    """
    df = scores[ (scores['metric']==metric_name) & (scores['set']==tr_set) ].reset_index(drop=True)
    df_mlt = df.melt(id_vars=['tr_size', 'metric'], value_vars=[c for c in df.columns if prfx in c])
    df_mlt = df_mlt.rename(columns={'value': 'scores'})

    # Plot all scores
    ax = None
    ax = plot_lrn_crv_new(
        x=df_mlt['tr_size'], y=df_mlt['scores'], yerr=None,
        ax=ax, ls='', marker='.', alpha=0.7, metric_name=metric_name,
        **plot_args, color='b', label='All scores');

    # Plot median
    ax = plot_lrn_crv_new(
        x=df['tr_size'], y=df['median'], yerr=None,
        ax=ax, ls='--', marker='.', alpha=0.7, metric_name=metric_name,
...
def plot_lrn_crv_new(x, y, yerr=None, metric_name:str='score',
    ...
    """
    TODO: rename this func globally to plot_lc()
